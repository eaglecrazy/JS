...
class Chat {
    ...
    addEvents() {
        const me = this;
        this.cross.addEventListener('click', (evt) => {
            evt.stopPropagation(); //прекращаем обработку события иначе будет ещё один клик на хидер
            me.changeState();
        });
        ...
        this.textarea.addEventListener('keydown', evt => {
                ...
                if (me.textarea.value === '' || me.textarea.value === '\n' || me.textarea.value === '\r\n') {
                    me.textarea.value = "";
                    return;
                }

                let message = new Message(me.textarea.value.trim(), new Date(), me.user);
        ...
        changeState() {
            this.chatWrapper.classList.toggle('chatCollapsed');
        ...
        render() {
            ...
            let str = `<div class="chatWrapper ${collapsed}">
                        <div class="chatHeader">
                            ${this.chatCrossRender()}
                            ${this.chatAvatarRender()}
                            <span class="chatName">${this.admin.name}</span>
                            <span class="chatInfo">Напишите нам, мы онлайн!</span>
                            <span class="chatWorker">${this.admin.position}</span>
                        </div>
